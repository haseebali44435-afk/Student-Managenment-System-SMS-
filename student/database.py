...
class StudentApp(ctk.CTk):
    ...
    def _load_from_db(self):
        """Load all records from the database into self.students dict."""
        self.students = {}
        cur = self.conn.cursor()

        # Load students
        for row in cur.execute(
            "SELECT roll, name, department, semester, email, phone, fees_status, fees_amount FROM students"
        ):
            roll, name, dept, sem, email, phone, fs, fa = row
            self.students[roll] = {
                "name": name, "department": dept, "semester": sem,
                "email": email, "phone": phone,
                "fees_status": fs, "fees_amount": fa,
                "attendance": {}, "marks": {},
            }

        # Load attendance
        for roll, date, status in cur.execute("SELECT roll, date, status FROM attendance"):
            if roll in self.students:
                self.students[roll]["attendance"][date] = status

        # Load marks
        for roll, subject, m in cur.execute("SELECT roll, subject, marks FROM marks"):
            if roll in self.students:
                self.students[roll]["marks"][subject] = m

    # ---- Per-record DB helpers ----

    def _db_upsert_student(self, roll):
        """Insert or replace a student core record (no marks/attendance)."""
        s = self.students[roll]
        self.conn.execute(
            """INSERT OR REPLACE INTO students
               (roll, name, department, semester, email, phone, fees_status, fees_amount)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
            (roll, s["name"], s["department"], s["semester"],
             s["email"], s["phone"], s["fees_status"], s["fees_amount"])
        )
        self.conn.commit()

    def _db_rename_student(self, old_roll, new_roll):
        """Rename a student's roll number (cascade re-insert because PK change)."""
        s = self.students[new_roll]
        # Insert new row
        self.conn.execute(
            """INSERT OR REPLACE INTO students
               (roll, name, department, semester, email, phone, fees_status, fees_amount)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
            (new_roll, s["name"], s["department"], s["semester"],
             s["email"], s["phone"], s["fees_status"], s["fees_amount"])
        )
        # Move attendance & marks to new roll
        self.conn.execute("UPDATE attendance SET roll=? WHERE roll=?", (new_roll, old_roll))
        self.conn.execute("UPDATE marks      SET roll=? WHERE roll=?", (new_roll, old_roll))
        # Delete old student row
        self.conn.execute("DELETE FROM students WHERE roll=?", (old_roll,))
        self.conn.commit()

    def _db_delete_student(self, roll):
        """Delete student + all cascade attendance & marks."""
        self.conn.execute("DELETE FROM students WHERE roll=?", (roll,))
        self.conn.commit()

    def _db_upsert_attendance(self, roll, date, status):
        self.conn.execute(
            "INSERT OR REPLACE INTO attendance (roll, date, status) VALUES (?, ?, ?)",
            (roll, date, status)
        )
        self.conn.commit()

    def _db_upsert_all_attendance(self, date):
        """Bulk-save every student's attendance for one date."""
        rows = [
            (roll, date, s["attendance"].get(date, "Absent"))
            for roll, s in self.students.items()
        ]
        self.conn.executemany(
            "INSERT OR REPLACE INTO attendance (roll, date, status) VALUES (?, ?, ?)", rows
        )
        self.conn.commit()

    def _db_upsert_marks(self, roll, subject, marks_val):
        self.conn.execute(
            "INSERT OR REPLACE INTO marks (roll, subject, marks) VALUES (?, ?, ?)",
            (roll, subject, marks_val)
        )
        self.conn.commit()

    def _db_delete_mark(self, roll, subject):
        self.conn.execute("DELETE FROM marks WHERE roll=? AND subject=?", (roll, subject))
        self.conn.commit()

    def _db_update_fees(self, roll):
        s = self.students[roll]
        self.conn.execute(
            "UPDATE students SET fees_status=?, fees_amount=? WHERE roll=?",
            (s["fees_status"], s["fees_amount"], roll)
        )
        self.conn.commit()

    def _db_update_all_fees(self, status):
        self.conn.execute("UPDATE students SET fees_status=?", (status,))
        self.conn.commit()

    def _on_closing(self):
        """Close DB connection cleanly before exiting."""
        try:
            self.conn.close()
        except Exception:
            pass
    ...
    def add_student(self):
        ...
        self._db_upsert_student(roll)          # ← persist to DB
    ...
    def update_student(self):
        ...
        old = self.students.pop(old_roll)
        old.update({"name": name, "department": self.sf_dept.get().strip(),
                    "semester": self.sf_sem.get().strip(), "email": self.sf_email.get().strip(),
                    "phone": self.sf_phone.get().strip()})
        self.students[roll] = old
        if roll == old_roll:
            self._db_upsert_student(roll)      # ← same roll: simple update
        else:
            self._db_rename_student(old_roll, roll)  # ← roll changed: migrate
        self.selected_student_roll = None
        self.refresh_students_table()
        self.clear_student_form()
    ...
    def delete_student(self):
        ...
        if messagebox.askyesno("Confirm", f"Delete student '{name}'?"):
            ...
            self._db_delete_student(roll)      # ← persist to DB
            self.refresh_students_table()
            self.clear_student_form()
    ...
    def toggle_attendance(self, _event):
        ...
        self.students[roll].setdefault("attendance", {})[date] = new_status
        self._db_upsert_attendance(roll, date, new_status)  # ← persist to DB
    ...
    def mark_all_att(self, status):
        ...
        self._db_upsert_all_attendance(date)    # ← bulk persist to DB
        self.load_attendance_list()

    def save_attendance(self):
        """Explicit save button – bulk-writes in-memory attendance for this date to DB."""
        date = self.att_date.get().strip()
        self._db_upsert_all_attendance(date)
        messagebox.showinfo("Saved", f"Attendance for {date} saved to database!")
    ...
    def add_marks(self):
        ...
        self._db_upsert_marks(roll, subj, val)  # ← persist to DB
        self.refresh_marks()
    ...
    def delete_marks(self):
        ...
        if roll in self.students and subj in self.students[roll]["marks"]:
            ...
            self._db_delete_mark(roll, subj)    # ← persist to DB
            self.refresh_marks()
    ...
    def update_fees(self):
        ...
        self._db_update_fees(roll)              # ← persist to DB
        self.refresh_fees()
    ...
    def mark_all_paid(self):
        if messagebox.askyesno("Confirm", "Mark ALL students as Paid?"):
            ...
            self._db_update_all_fees("Paid")    # ← bulk persist to DB
            self.refresh_fees()
    ...
    def mark_all_pending(self):
        if messagebox.askyesno("Confirm", "Mark ALL students as Pending?"):
            ...
            self._db_update_all_fees("Pending") # ← bulk persist to DB
            self.refresh_fees()
    ...
    def build_settings(self):
        ...
        ctk.CTkLabel(ab,
                     text="Student Management System  v2.0  (SQLite Edition)\n"
                          "Built with Python 3 & CustomTkinter\n"
                          "Database: students.db  |  Features: Students · Attendance · Marks · Fees · Analytics",
    ...
    def _clear_data(self):
        if messagebox.askyesno("Confirm", "Delete ALL student data from the database? This cannot be undone!"):
            self.conn.execute("DELETE FROM marks")
            self.conn.execute("DELETE FROM attendance")
            self.conn.execute("DELETE FROM students")
            self.conn.commit()
            self.students = {}
            messagebox.showinfo("Cleared", "All data has been cleared from the database.")
